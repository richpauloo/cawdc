...
This is a copy of an older script `06_calibration.Rmd`, and the only difference is in the calibration. In this script, the calibration of `d` takes place from the land surface to the bottom of the perforated interval, which is more physically meaningful than in the former model specification, in which `d` was set between the water level at Spring 2011 and the bottom of the perforated interval.  

The other suggested difference was to minimize the error from the **count** of well failures in an areal unit, rather than the **percentage**. Because we only use townships with n >= 60 samples, using count rather than percentage will only have the effect of giving more weight to  areal units with a greater number of wells. 
