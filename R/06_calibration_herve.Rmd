...
domcv6 <- domcv6[domcv6@data$wch_2 >= 0 &
                 domcv6@data$Ensemble <= domcv6@data$bot, ]

# re-transform
domcv6 <- spTransform(domcv6, merc)

# make a copy for the optimization
domcv10 <- domcv8 <- domcv7 <- domcv6
```
...
# find quartiles
quart2 <- quantile(oswcr_in_calibration, c(.5, 1))
...
The simple model under-predicts actual failure rates for every calibration township. This is because wells dry out before the water level reaches the well bottom, which we now incorporate into the model. 
...
We calibrate the pump separation proportion, `d` with single parameter optimization and `optimize()`.
```{r}
...
fun <- function(x, w) {
  ...
  domcv11 <- raster::extract(emax,     # extract from emax
                           domcv10,    # to domcv points
                           sp = TRUE)# and add the values to a data frame

  # remove the ~1000 wells that fall outside of the raster cells
  domcv11 <- domcv11[!is.na(domcv11@data$max_gw), ] 
  
  # copy of the data without missing values and imposisble dates
  domcv11 <- domcv11[which(!is.na(domcv11@data$year) & 
                        domcv11@data$year <= 2017 &  # remove impossible values
                        domcv11@data$year >= 1900), ]# out of range
  
  # ry <- 2012 - a            
  ry <- cutoff
  
  # subset for active wells
  domcv11 <- domcv11[which(domcv11@data$year >= ry), ] 
  
  # water column heights
  # negative water column heights do not affect optimization
  # these are just wells that are very far gone
  wch <- domcv11@data$bot - 0
  ...
  # put into dataframe and compute well failures
  domcv11@data <- domcv11@data %>% 
    mutate(pl = pl,
           dry = ifelse(max_gw >= pl, TRUE, FALSE))

  # count dry and wet wells in calibration townships
  town_dry <- poly.counts(domcv11[domcv11@data$dry == T, ], calibration_poly2)#dry
  town_wet <- poly.counts(domcv11[domcv11@data$dry == F, ], calibration_poly2)#wet
