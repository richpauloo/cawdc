...
Only include the polygons with sufficiently large area (remove boundary polygons with small area).
```{r}
...
# write_rds(dry_12_16, "dry_12_16.rds")
...
Run this script through line 205.  
...
# remove wells that are already dry, about 20% of data
dry_12_16 <- read_rds("dry_12_16.rds")
...
# Alvar's Sustainable Groundwater Management Scenarios
...
r <- lapply(r, projectRaster, crs = merc)

s1 <- r[[1]] # 1 year drought
s2 <- r[[2]] # 2 year drought
s3 <- r[[3]] # 3 year drought

# initial conditions = mean(spring/fall 2017)
sp_fa_2017_gwl <- read_rds(here("code","sp_fa_2017_gwl.rds"))
sp_fa_2017_gwl <- projectRaster(sp_fa_2017_gwl, crs = merc)
...
# add scenario GWL to spatial points (wells)
# extract scenario water levels
domcv <- raster::extract(s1,       # extract from 
                         domcv,    # to domcv points
                         sp = TRUE)# and add the values to a data frame

domcv <- raster::extract(s2,       # extract from 
                         domcv,    # to domcv points
                         sp = TRUE)# and add the values to a data frame

domcv <- raster::extract(s3,       # extract from 
                         domcv,    # to domcv points
                         sp = TRUE)# and add the values to a data frame


# remove the ~1000 wells that fall outside of the raster cells
domcv <- domcv[!is.na(domcv@data$r2020) & 
               !is.na(domcv@data$r2040) &
               !is.na(domcv@data$r2040gp), ] 
...
# put into dataframe and compute well failures
domcv3@data <- domcv3@data %>% 
  mutate(dry_s1 = ifelse(r2020 >= pump_loc, TRUE, FALSE), # wells dry in s1 
         dry_s2 = ifelse(r2040 >= pump_loc, TRUE, FALSE), # wells dry in s2 
         dry_s3 = ifelse(r2040gp >= pump_loc, TRUE, FALSE)) # wells dry in s3 
        

# sanity check
list(domcv3@data$dry_s1, domcv3@data$dry_s2, 
     domcv3@data$dry_s3) %>% 
  lapply(., table)

# list of results
z <- list(domcv3[domcv3@data$dry_s1==T,],
     domcv3[domcv3@data$dry_s2==T,],
     domcv3[domcv3@data$dry_s3==T,])

# write files
# write_rds(z, "sustainable_gw_mgmt_pred.rds")
...
# z <- lapply(z, spTransform, ll)
# export
# shapefile(z[[1]], filename = "s1_2020.shp")
# shapefile(z[[2]], filename = "s2_2040.shp")
# shapefile(z[[3]], filename = "s3_2040_gp.shp")
# shapefile(z[[4]], filename = "d4.shp")
...
# B118 
b118_dry_s1 <- poly.counts(domcv3[domcv3$dry_s1 == T, ], b118cv)
b118_wet_s1 <- poly.counts(domcv3[domcv3$dry_s1 == F, ], b118cv)
b118_frp <- b118_dry / (b118_dry + b118_wet)
