...
# save for 10_space_time_kriging
#write_rds(dl_cv, "dl_cv_log_trans.rds")
...
# #
# for(i in 1:12){
#   p <- data.frame(x = el[[i]]@data@values) %>% 
#     ggplot(aes(x)) +
#     geom_density() +
#     labs(title = paste(title[i], "Groundwater Depth Below Land Surface (ft)")) +
#     coord_cartesian(ylim = c(0, 0.01), xlim = c(0, 650))
#  ggsave(p, filename = paste0(formatC(i, width = 2, flag = "0"), title[i],".png"))
# }

# boxplot of spring to fall GW depth below land surface relationships
l <- list()
for(i in 1:12){
  l[[i]] <- data.frame(t = title[i],
                       v = el[[i]]@data@values)
}

ldf <- do.call(rbind.data.frame, l) %>% filter(!is.na(v))
ldf$season <- grepl(ldf$t, pattern= "SP")
ldf$season <- ifelse(ldf$season == TRUE, "Spring", "Fall")

gw_boxplots_sp_fall <- ldf %>% 
  mutate(v = -1*v*0.3048) %>%  # convert feet to meters, and make negative
  ggplot(aes(t, v)) +
  geom_boxplot(outlier.shape = NA, coef = 0, aes(fill = season), alpha = 0.5) +
  coord_cartesian(ylim = c(-47,0)) +
  # scale_fill_viridis_d(option = "viridis") +
  scale_fill_manual(values = rev(colormap(colormaps$viridis, nshades=2))) +
  labs(x = "Time", y = "Depth to groundwater (m)", fill = "Season") +
  theme_set(theme_minimal(base_size = 16)) + theme(legend.position = "bottom")

gw_boxplots_sp_fall
# save
ggsave(gw_boxplots_sp_fall, filename = "gw_boxplots_sp_fall.pdf", device = cairo_pdf)
```
Plot Spring 2012 and Fall 2016 GWL.
```{r}
...
#ggsave(p, filename = "test.pdf", device = cairo_pdf, height = 4, width = 8)
```
