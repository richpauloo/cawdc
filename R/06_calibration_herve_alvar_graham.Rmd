...
data.frame(obs = calibration_fr2, pred = frp) %>% 
  ...
  coord_cartesian(ylim = c(0.025,.2), xlim = c(0.025,.2)) + 
  theme_minimal() +
...
p1 <- ggplot(df) +
  geom_point(aes(ret_age, error))
  
p2 <- ggplot(df) +
  geom_point(aes(ret_age, n_dry)) +
  geom_hline(yintercept = 2000)

cowplot::plot_grid(p1, p2)
```

Retirement age of 29 years gives the lowest SSE. Go back to top and plug this in.
...
Re-compute the model with $\omega = 0.98$
```{r}
# remove wells that are already dry, about 20% of data
domcv@data <- domcv9@data %>% 
mutate(dry = ifelse(Ensemble >= pump_loc, TRUE, FALSE)) # wells dry in SP 2011
...
# apply water level scaling factor and extract level for each of point
emax <- mean(es) * .98 # mean groundwater depth below land surface 
names(emax) <- "max_gw" # change the name of the raster layer
...
pal <- colorBin(palette = colormap(colormaps$viridis, nshades = 5),
                domain = gsacvsf$frp, bins = seq(0,100,20))

pal2 <- colorBin(palette = colormap(colormaps$jet, nshades = 10),
                 domain = gsacvsf$dry, bins = seq(0,400,40))
...
pal <- colorBin(palette = colormap(colormaps$viridis, nshades = 4),
                domain = b118cvsf$frp, bins = seq(0,100,25))

pal2 <- colorBin(palette = colormap(colormaps$jet, nshades = 10),
                 domain = b118cvsf$dry, bins = seq(0,700,70))
